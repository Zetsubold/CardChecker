...
#include "logging.h"
#include "charafemale.h"

using namespace std;

Female::Female(ifstream &f) : Thing(f), file(f)
{
}
...
bool Female::read_string(int &count)
{
	store_data(file, count);
	if (file.eof()) {
		logfile << "Female::process -- ERROR in reading item text.\n\t-- tried reading ";
...
bool Female::process(Midpoint & midpoint)
{
	int count;
	...
	store_data(file, 636, true);

	return true;
}


ostream& operator<< (ostream & out, const Female & in)
{
...
bool Female::write(std::ofstream & out)
{
	...
	*(float *)(data[61]) = z_scal;
	...
	logfile << "Female::write -- wrote " << out.tellp() - pos << " bytes." << endl;
